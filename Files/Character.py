...
class Character:
    def __init__(self, name, score, map, biome_map, collect_time_map,  health, energy, inventory, items, hand_item, hand_item_slot, armor, location, orientation, last_move_time):
        self.name = name
        self.score = score
        self.map = map
        ...
        try:
            if self.orientation == "North":
                biome_ahead = self.biome_map[self.location[0] -
                                             2][self.location[1]-1].name
            elif self.orientation == "South":
                biome_ahead = self.biome_map[self.location[0]
                                             ][self.location[1]-1].name
            elif self.orientation == "West":
                biome_ahead = self.biome_map[self.location[0] -
                                             1][self.location[1]-2].name
    ...
    def describe_spawnpoint(self, game):
        ...
        print("You spawn %s %s. You are facing North." %
              (alt_preposition, current_biome))
    ...
    def move(self, game):
        ...
        try:
            ...
            print("You move %s %s %s." % (self.orientation, eval(
                "game." + current_biome).preposition, current_biome))
        except IndexError:
    ...
    def collect(self):
        if time() - self.collect_time_map[self.location[0]-1][self.location[1]-1] < COLLECT_DELAY:
            return
        NewResources = self.biome_map[self.location[0] -
                                      1][self.location[1]-1].gen_resources()
    ...
    def attack(self):
        if self.hand_item == None:
            print("You punch with your fist at thin air.")
